...
control MyEgress(inout headers hdr,
    ...
    apply {
        if(hdr.int_header.isValid()) {
            ...
            if (local_metadata.int_meta.sink == _TRUE) {                        // Switch is a Sink, packet is not a mirror
                process_int_sink.apply(hdr, local_metadata);
                
            } else if (local_metadata.int_meta.sink == _FALSE) {                // Switch is a Sink, packet is a mirror, or switch is not a sink
            ...
            if (local_metadata.mirror == _TRUE) {       // Packet is a mirror (only on the sink)
