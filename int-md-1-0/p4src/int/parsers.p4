...
parser MyIngressParser(packet_in packet,
    ...
    // Need to parse INT data here, but the length of the field needs to be known by looking at the shim header
...
parser MyEgressParser(packet_in packet,
    ...
    state parse_int_data {
        // packet.extract(hdr.int_data); 
        // if INT node is a source or transit node, no need to extract int data here. only for sink node. 
        packet.extract(hdr.int_data);
    ...
    // size of packet advance needs to be determined by values in the header
    state parse_advance {
